                ...
                <div class="add_con">
                    ...
                    <ul class="add_question_ul add_single_ul" v-if="radioVal == '单选'">
                        <li class="add_model_con_row" style="position:relative;">
                            ...
                            <ul class="addLikeQuery" v-show="likeQueryShow" style="position: absolute;top: 32px;left: 63px;max-height: 80px;overflow: auto;background: #fff;width: 230px;border: 1px solid #3967AE;border-radius: 3px;">
                    ...
                    <div class="fill_inthe_blank" v-else-if="fill_inthe_blank_isShow">
                        ...
                        <div>
                            ...
                            <input type="radio" name="blankType" value="txt" checked> 文本
                ...
                <div class="add_con">
                    ...
                    <div>
                        <ul class="add_question_ul modify_question_ul" v-show="add_question_ul_isShow">
                            <li class="add_model_con_row"><span>题目：</span><input type="text" id="topic2" maxlength="255"></li>
                            <li class="add_model_con_row"><span>A选项：</span><input type="text" id="A2" data="A" maxlength="255"></li>
                            <li class="add_model_con_row"><span>B选项：</span><input type="text" id="B2" data="B" maxlength="255"></li>
                            <li class="add_model_con_row"><span>C选项：</span><input type="text" id="C2" data="C" maxlength="255"></li>
                            <li class="add_model_con_row"><span>D选项：</span><input type="text" id="D2" data="D" maxlength="255"></li>
                            <li class="add_model_con_row"><span>E选项：</span><input type="text" id="E2" data="E" maxlength="255"></li>
                        </ul>
                        <div style="display: inline-block;width: 50px;" v-if="radioVal2 == '多选'">
                            <img src="../../static/circle-add.png" alt="" @click="modifyAddOpt">
                            <img src="../../static/circle-minus.png" alt="" @click="modifyMinusOpt">
                        </div>
                    </div>
                    <div class="fill_inthe_blank" v-show="fill_inthe_blank_isShow">
                        <div>
                            <span>题目:</span>
                            <q id="fillintheblank2" @keydown="keyAction($event,255)" style="outline: none;display:inline-block;width:270px;border-bottom: 1px solid #000;" contenteditable="true">
...
export default {
    ...
    methods: {
        ...
        modifyAddOpt(){    //修改题目 添加多选题选项
            var num = $(".modify_question_ul li").length-1;
            console.log(num);
            if(num<26){
                var charCode = String.fromCharCode(65+num)
                console.log(charCode);
                $(".modify_question_ul").append("<li class='add_model_con_row' style='padding:7px 0;'><span style='display: inline-block;width: 58px;text-align: right;'>"
                    +charCode+"选项：</span><input type='text' style='width: 230px;border: 1px solid #b5b5b5;border-radius: 3px;padding: 3px 10px;' id='"
                    +charCode+"' value='' placeholder='请输入选项内容' maxlength='255'></li>");
            }
            
        },
        modifyMinusOpt(){   //修改题目 删减多选题选项
            var num = $(".modify_question_ul li").length-1;
            console.log(num);
            if(num>2){
                $(".modify_question_ul").children("li:last-child").remove();
            }
        },
        problemShow(){
            //添加
        ...
        modifyShow(){
            ...
            if($("#result_table tr td input").length == 0){
                ...
                if($('#'+this.questionId).text() != '0'){   //使用次数不为0，当前题目已被使用
                    this.$dialog.alert("题目已被问卷使用，不能修改")
                }else {
                    this.isShow2 = true
                    let param = new URLSearchParams()
                    ...
                    this.axios({
                        ...
                        if(res.data.e == 0){
                            ...
                            if(questionType == "单选"){
                                $('#name5,#labelName5,#name6,#labelName6').addClass('hide')
                                $('#name4,#labelName4').removeClass('hide')
                                this.radioVal2 = "单选"
                                ...
                                var html = "<li class='add_model_con_row' style='padding: 7px 0;'>"
                                    ...
                                    +"<input type='text' id='topic2' style='width: 230px;border: 1px solid #b5b5b5;border-radius: 3px;padding: 3px 10px;' maxlength='255'></li>";
                                for(var i=0;i<answerOptions.length;i++){
                                    html += "<li class='add_model_con_row' style='padding: 7px 0;'><span style='display: inline-block;width: 58px;text-align: right;'>"
                                        ...
                                        +answerOptions[i]+"' value='"+answerOptionsValue[i]+"' maxlength='255'></li>"
                                }
                                ...
                                $('#name4,#labelName4,#name6,#labelName6').addClass('hide')
                                $('#name5,#labelName5').removeClass('hide')
                                this.radioVal2 = "多选"
                                var answerOptions = res.data.o[0].answerOptions.split(',');
                                var answerOptionsValue = res.data.o[0].answerOptionsValue.split(',');
                                var html = "<li class='add_model_con_row' style='padding: 7px 0;'>"
                                ...
                                $('#name4,#labelName4,#name5,#labelName5').addClass('hide')
        ...
        add(){
            ...
            if(this.radioVal == "多选"){
                ...
                for(var i=1;i<$('.add_opt_ul li input').length;i++){
                    if($('.add_opt_ul li input')[i].value == ''){
                        this.$dialog.alert("请将选项内容填写完整");
                        return;
                    }
            ...
            if(this.radioVal == "填空"){
                var questionName = $('#fillintheblank1').text();    //添加填空题题目内容
                //var fillingType = $('#setUp1').val()
                var fillingType = $("input[name='blankType']:checked").val();     //添加填空题 填空类型设置
                if(questionName == "" || questionName == "________"){
                    this.$dialog.alert("题目名称必须填写")
                    return false
                }
                if(fillingType == ""){
                    this.$dialog.alert("填空类型必须设置")
                    return false
                }
                let param = new URLSearchParams()
                param.append('questionName', questionName)
        ...
        modify(){
            ...
            if(this.radioVal2 == "多选"){
                ...
                for(var i=1;i<$('.modify_question_ul li input').length;i++){
                    if($('.modify_question_ul li input')[i].value == ''){
                        this.$dialog.alert("请将选项内容填写完整");
                        return;
                    }
                    if(answerOptionsValue == ''){
