<template>
  <div class="template">
    ...
    <div class="qrcode" v-show="qrcodeShow">
        <img :src="qrcodeSrc" alt="" style="width:200px;height:200px;"/>  
        <p>请识别二维码 关注公众号!</p>
    </div> 
    <div v-show="!qrcodeShow" class="template_body"
    :class="{
    ...
    <div class="template_title"
       ...
       <div class="template_body_content">
           ...
           <div class="submit" v-show="!qrcodeShow">
...
export default {
  data(){
    return {
      ...
      openId: '',
      questionnaireStyleId: '',
      qrcodeSrc: ''
    }
  },
  mounted(){
    this.initWx()
    this.initializationLoad()
  },
  methods: {
    initializationLoad(){
      let param = new URLSearchParams()
      param.append('code',this.GetQueryString('code'))
      param.append('state',this.GetQueryString('state'))
      this.axios({
          method: 'post',
          url: 'SharingProcess.do?SharingProcessDemo',
          data: param,
      }).then(res=>{
         if(res.data.e == 0){
           this.$dialog.alert("您暂无答题权限").then(function(){
              wx.closeWindow();
           })
         }else if(res.data.e == 1){
           this.$dialog.alert("您已经回答过!不可重复答题").then(function(){
              wx.closeWindow();
           })
         }else if(res.data.e == 2){
           var dataStr = res.data.o.split(',')
           this.openId = dataStr[0]
           this.questionnaireId = dataStr[1]
           this.questionnaireStyleId = dataStr[2]
           this.initQuestionTemplate()
           this.initTemplate()
         }else if(res.data.e == 3){
           this.$dialog.alert("请您先关注公众号,再进行答题！").then(function(){
              wx.closeWindow();
           })
         }else {
            this.qrcodeShow = true
            this.qrcodeSrc = res.data.o
         }
      })
    },
    initQuestionTemplate(){
        if(this.questionnaireStyleId  == '1'){
            this.template1 = true
        ...
        if(this.questionnaireStyleId  == '2'){
        ...
        if(this.questionnaireStyleId  == '3'){
            this.template1 = false
            this.template2 = false
            this.template3 = true
            this.template4 = false
        ...
        if(this.questionnaireStyleId  == '4'){
            this.template1 = false
            this.template2 = false
            this.template3 = false
            this.template4 = true
        ...
        if(this.questionnaireStyleId  == '5'){
            this.template1 = false
            this.template2 = false
            this.template3 = false
            this.template4 = false
