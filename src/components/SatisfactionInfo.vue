<template>
    <div class="main_box">
      <v-title>
        ...
        <li class="active" slot="name">{{nairName}}</li>
      </v-title>
      <div class="export">
      ...
      <div class="result_analyse">
        ...
        <div class="subject" :id="item[0].questionNameType.questionnamevalId" v-for="(item,index) in SatisfactionInfo" v-show="!noDataShow">
            ...
            <div class="subject_left">
                ...
                <div class="tab" v-else-if="item[0].questionNameType.questionNameTypemc=='填空,整数'||item[0].questionNameType.questionNameTypemc=='填空,小数'">
                    <table class="table table-bordered">
                      <tbody>
                ...
                <div class="tab" v-else-if="item[0].questionNameType.questionNameTypemc=='填空,文本'">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th>答题者</th>
            ...
            <div class="subject_right" :id="index">
                ...
                ||item[0].questionNameType.questionNameTypemc=='填空,整数'
                ||item[0].questionNameType.questionNameTypemc=='填空,小数'">
                ...
                v-if="item[0].questionNameType.questionNameTypemc=='单选'
                ||item[0].questionNameType.questionNameTypemc=='填空,整数'
                ||item[0].questionNameType.questionNameTypemc=='填空,小数'"></div>
                
                <div 
                :id="'echartsZhu'+index" 
                style="height: 255px;width: 600px;margin: 0 auto;" 
                v-if="item[0].questionNameType.questionNameTypemc=='多选'"></div>
            </div>
...
export default {
    ...
    methods: {
                ...
                data: param,
                 ...
                 for(let i=0;i<res.data.o.length;i++){
                    ...
                    if(name == "单选"){
                        ...
                        // }
                        var singleOption = res.data.o[i][0].data.content.split('~@~')
                        res.data.o[i][0].data=[];
                        ...
                        for(var j=0;j<singleOption.length;j++){
                            ...
                            res.data.o[i][0].echartsDataX.push(singleOption[j].split(':')[1].split(',')[0])
                            res.data.o[i][0].echartsDataY.push(singleOption[j].split(':')[1].split(',')[1])
                        }
                        setTimeout(()=>{
                           this.dropEcharts(res.data.o[i][0].echartsDataX,res.data.o[i][0].echartsDataY,i);
                        },150)
                    }else if(name=="填空,整数"||name=="填空,小数"){
                        // var singleOption = [];
                        // for(var key in res.data.o[i][0].data){
                        //     singleOption.push(res.data.o[i][0].data[key])
                        // }
        ...
        dropEcharts(echartsDataX,echartsDataY,id){
             var myChart = this.$echarts.init(document.getElementById('echartsZhu'+id));
             myChart.setOption({
                tooltip : {
                    trigger: 'item'
                },
        ...
        exportEcharts(){
            ...
            if(this.questionId == ""){  //页面无数据
                ...
                for(let i = 0;i<this.questionId.length;i++){
                    ...
                    }else if(questionNameTypemc == "填空,小数题" || questionNameTypemc=="填空,整数题"){
