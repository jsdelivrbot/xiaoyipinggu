<template>
  <div>
        ...
        <table id="modelTable" class="table table-responsive">
            ...
            <tbody>
                <tr v-for="(item,index) in initData" v-show="!noDataIsShow">
                    ...
                    <td :id="item.formulaNum" :title="item.titleTypeValue">{{item.titleTypeValue}}</td>
                </tr>
    ...
    <div class="bg_box" v-show="isShowModify">
        <div id="modify_model">
            <div class="add_model_title">修改模型
                <span class="glyphicon glyphicon-remove add_model_close"
            ...
            <div class="add_model_con">
                ...
                <div>
                    <span id="txtModifyTitle1" class="" style="display: inline-block;">模型公式:</span>
                    <input type="text" 
                    id="txtModify1"
                    class="form-control txt"
                    onpaste="return false" 
                    ...
                    <span id="txtModifyTitle2" class="hide" style="margin: 10px 0;display: inline-block;">填空模型题号:</span>
                    <input type="text" 
                    id="txtModify2"
                    class="form-control txt2 hide"
                    onpaste="return false" 
                    ondragenter="return false" 
                    oncontextmenu="return false;"  
                    onkeypress = "ValidateValue2(this)" 
                    ...
                    placeholder="T1,T2"
                    style="display:inline-block;width: 80%;margin: 10px 0;">
                    <div class="title_modify2 hide">
                        提示：请以T+填空题序号以,号分隔的形式填写
                    </div>
                    <br>
                    <span id="txtModifyTitle3" class="hide">填空模型基数:</span>
                    <input type="text" 
                    id="txtModify3"
                ...
                <div>
                    <p style="color: red;" class="hide" id="redModify"></p>
                    <p style="color: green;" class="hide" id="greenModify"></p>
                </div>
                <div class="add_model_btns">
...
export default {
    ...
    methods: {
            ...
            this.axios({
                ...
                if(res.data.e == 0){
                    for(var i=0;i<res.data.o.length;i++){
                        if(res.data.o[i].titleTypeValue == '填空题'){
                            res.data.o[i].generalFormula=res.data.o[i].generalFormula.split('=').join('*')
                        }
                    }
        ...
        addBtn(){
            ...
            this.textareaText=''
            $('#txtAdd2,#txtAdd3').val('')
            $('#greenAdd,#redAdd').addClass('active')
        },
        add(){
            let param = new URLSearchParams()
        ...
        modifyShow(){
            if($("#modelTable tr td input").length == 0){
                ...
                if($('#'+this.formulaNum).text() == "单选题"){
                    this.$dialog.alert("单选题模型不能进行修改")
                    return false
                }
        ...
        modify(){
            ...
            varval ="{"+varval+"}"
            varval=varval.replace("undefined","")
            ...
            this.axios({
                ...
                if(res.data.e == 0){
                    ...
                    this.$dialog.alert("模型名称已已被使用不能修改")
                }else {
