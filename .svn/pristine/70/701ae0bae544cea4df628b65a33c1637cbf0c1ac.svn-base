            ...
            <div>
                <table class="table table-responsive table-bordered" style="border: none;table-layout: fixed;">
                  <!-- 答案标题 -->
                    ...
                    <!-- 答案内容 -->
                    <tbody id="tbody">
                        <tr v-for="(item,index) in initTableVal" :id='index' :data="item.openId" class="topicTr" :class="item.answerId">
                            <td><input type="checkbox"  :value="item.answerId" v-model="checkIds"/></td>
                            <td>{{index + 1}}</td>
                            <td class="answerContent">{{item.userName}}</td>
                            <td class="result classTopic exam tc" :class="matchClass(index)" v-for="(item2,index) in item.answer" :title='item2.toString().substring(0,1) == "." ? parseFloat(item2) : item2'>{{item2.toString().substring(0,1) == "." ? parseFloat(item2) : item2}}</td>
  ...
  methods: {
    ...
    initTable() {
      ...
      this.axios({
        ...
        this.equation = JSON.stringify(res.data.o);
    ...
    submitFlag(){
      ...
      this.axios({
        ...
        if (res.data.e == 0) {
          var state  = res.data.o;
          if(state == '通过'){
            $('.submit_btn').removeClass('btn-success')
            $('.submit_btn').attr('disabled',true)
            $('.submit_btn').text('审核通过')
          }else if(state == '待审核'){
            $('.submit_btn').removeClass('btn-success')
            $('.submit_btn').attr('disabled',true)
            $('.submit_btn').text('待审核')
    ...
    back(){
       this.$router.push({path:'/benefitEvaluation',query:{index:1}})
    }
  },
  watch:{
      equation:function(){
