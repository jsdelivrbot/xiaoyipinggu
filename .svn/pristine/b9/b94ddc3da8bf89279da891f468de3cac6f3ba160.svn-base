  ...
  <div>
    <div class="bodys">
        ...
        <table id="nair_table" class="table table-responsive">
            ...
            <tr v-for="(item,index) in initData" v-show="!noDataIsShow">
                ...
                <td class="color3" :title="item.auditStatus" :id="item.questionnaireId" v-else-if="item.auditStatus =='审核通过' && item.groupHair != '1'">{{item.auditStatus}}</td>
                <td class="color3" :title="item.auditStatus" :id="item.questionnaireId" v-else-if="item.auditStatus =='审核通过' && item.groupHair == '1'">{{item.auditStatus}}(非群发)</td>
                ...
                <td style="white-space: inherit;display: inline-block;width: 212px;overflow: visible;">
                    ...
                    <span @click="preview(item.questionnaireId,item.questionnaireName.substring(0,2))">预览</span> |
                    <span v-if="item.auditStatus=='审核通过' && item.invalid == 0 && item.groupHair == '0' && item.sendFlag == 0" :id="item.questionnaireId+'Send'" @click="sendShow(item.questionnaireId,item.sendFlag)">发送</span>
                    ...
                    <div class="more">更多
                        <ul class="more_ul">
                            ...
                            <li v-if="item.auditStatus == '审核通过' && item.invalid != 1" @click="shareShow(item.questionnaireId)">分享</li>
                        </ul>
                    </div>
                </td>
            </tr>
        ...
        <div id="add_nair" class="add_model">
            ...
            <div class="add_con">
                <div class="add_model_con_row">
                    <span>问卷名称 ：</span>
                    <select id="addQuestionnaireVal" @change="questionnaireManageChange">
    ...
    <div class="bg_box" v-show="modifyNair">
        <div id="modify_nair" class="add_model">
            ...
            <div class="add_con">
                <div class="add_model_con_row">
                    <span>问卷名称 ：</span>
                    <select id="modifyQuestionnaireVal" @change="questionnaireManageChange">
                        <option value="公众">公众</option>
                        <option value="行业">行业</option>
                    </select>
    ...
    <div class="bg_box" v-show="isShowExamine">
        <div id="add_examine" class="add_model">
            ...
            <div class="add_con">
                <div class="add_model_con_row">
                    <input type="text" placeholder="请输入用户名" id="realNames"/>
    ...
    <div class="bg_box" v-show="isShowQrcode">
        <div class="add_model qrcode_model">
            ...
            <div class="add_con">
                ...
                <div>
                    <!-- <input type="text" v-model="qrcodeLink" id="copy_text" style="width:80%;">
                    <button class="btn btn-primary" style="padding: 3px 7px;" @click="">复制链接</button> -->
                    <input type="text" v-model="qrcodeLink" style="width:72%;">
                    <button class="btn btn-primary" @click="doCopy()">复制链接</button></div> 
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
    dataProcessing () {
        this.qrcodeLink = this.qrcodeLink
    },
    doCopy: function (val) {
        this.dataProcessing(val)
        var _this = this;
        this.$copyText(this.qrcodeLink).then(
            function (e) {   //success
                _this.$dialog.alert("复制成功");  
            },
    ...
    add_nair_show(){
        ...
        $("#addStopTime").val('')
        $("#reminderTime").val('')
        this.questionnaireManageChange()
    },
    ...
    modifyShow(num,id){
        ...
        if(num == 1){
            ...
            if($("#nair_table tr td input").length == 0){
                ...
                if($('#'+this.questionnaireId).text() == "待审核"){
                    this.$dialog.alert("待审核装填不能修改问卷")
                    return false
                }
                this.modifyNair = true
                let param = new URLSearchParams()
                ...
                this.axios({
                    ...
                    if(res.data.e == 0){
                       ...
                       // $('#modifyStartTimeHour').val(startTimeStr[1])
                        ...
                        //$('#modifyReminderTimeHour').val(reminderTimeStr[1])
                        this.questionnaireManageChange()
                        if($('#'+this.questionnaireId).text() == "审核未通过" || $('#'+this.questionnaireId).text() == "审核通过" || $('#'+this.questionnaireId).text() == "审核通过(非群发)"){
                            $('#modifyQuestionnaireVal').attr('disabled','disabled');
                            $('#modifyQuestionnaireName').attr('readonly','readonly');
                            $('#modifyQuestionnaireTyp').attr('disabled','disabled');
                        }else if($('#'+this.questionnaireId).text() == "未提交审核"){
                            $('#modifyQuestionnaireVal').removeAttr('disabled');
                            $('#modifyQuestionnaireName').removeAttr('readonly');
                            $('#modifyQuestionnaireTyp').removeAttr('disabled');
                        }
                    }
                })
            }
                    ...
                    success:(res)=>{
                        if(res.e == 0){
                            for(var i = 0;i < res.o.length;i++){
                                if(res.o[i].questionType != "填空"){
                                    var answerOptions = res.o[i].answerOptions
                                    if(answerOptions != null){
                                        var answerOptionsArr = answerOptions.split(',')
                                        var answerOptionsValue = res.o[i].answerOptionsValue
                                    ...
                                    for(var j = 0;j<answerOptionsArr.length;j++){
                                        ...
                                        var data = [
                                            {
                                            "answerOptions":null,"answerOptionsValue":null,"fillingType":res.o[i].fillingType
                                            }
                                        ]
                                        res.o[i].data= data
                                    }
                                    this.initData2 = res.o
                            }
                            ...
                            this.initData2 = []
                            this.$dialog.alert("当前问卷为空，请添加题目后再进行提交")
                        }
                    }
                })
            }
    ...
    shareShow(id){
        this.shareId = id
        console.log($('#'+id).text());
        if($('#'+id).text() == "审核通过" || $('#'+id).text() == "审核通过(非群发)"){
    ...
    jumpTopicManage(id,name,type){
        //跳转到题目管理
        if($('#'+id).text() == '待审核' || $('#'+id).text() == '审核通过' || $('#'+id).text() == "审核通过(非群发)"){
    ...
    preview(id,questionnaireNameType){
      //预览
      this.$router.push({path:'/template?index=template1',query:{id:id,questionnaireNameType:questionnaireNameType}})
    },
    questionnaireManageChange(){
        if($('#addQuestionnaireVal').val() == '公众'){
            $('#add_nair .add_model_con_row').eq(3).addClass('hide')
        }else {
            $('#add_nair .add_model_con_row').eq(3).removeClass('hide')
        }
        if($('#modifyQuestionnaireVal').val() == "公众"){
            $('#modify_nair .add_model_con_row').eq(3).addClass('hide')
        }else {
            $('#modify_nair .add_model_con_row').eq(3).removeClass('hide')
        }
    },
    stateCommit(){
